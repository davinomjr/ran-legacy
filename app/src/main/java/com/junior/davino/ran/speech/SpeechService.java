...
public class SpeechService extends Service {
    ...
    private final StreamObserver<SyncRecognizeResponse> mResponseObserver
            = new StreamObserver<SyncRecognizeResponse>() {
        @Override
        public void onNext(SyncRecognizeResponse response) {
            Log.i(TAG, "onNext RESPONSEEE");
            String text = null;
            boolean isFinal = true;
            Log.i(TAG, "RESPONSE COUNT = " + response.getResultsCount());
            if (response.getResultsCount() > 0) {
                final SpeechRecognitionResult result = response.getResults(0);
                if (result.getAlternativesCount() > 0) {
                    for (SpeechRecognitionAlternative alterinative : result.getAlternativesList()) {
                        Log.i(TAG, "ALTERNATIVE RECOGNIZED = " + alterinative.getTranscript());
                    }
...
//
//        // Configure the API
//        mApi.syncRecognize(req, mResponseObserver);
...
//                                .build())
    ...
    private class AccessTokenTask extends AsyncTask<Void, Void, AccessToken> {
        ...
        protected AccessToken doInBackground(Void... voids) {
            ...
            // *******************
            final InputStream stream = getResources().openRawResource(R.raw.apiprojectd25b94808fd6);
            try {
